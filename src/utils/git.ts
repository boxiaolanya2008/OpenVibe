...
export function parseGitUrl(source: string): GitSource | null {
	...
	if (!hasGitPrefix && !/^(https?|ssh|git):\/\//.test(url)) {
		return null;
	}
	...
	const httpsCandidates = [split.ref ? `https://${url.replace(/^\w+:\/\//, "")}#${split.ref}` : undefined, url].filter(
		(value): value is string => Boolean(value),
	);
	for (const candidate of httpsCandidates) {
		...
		if (info) {
			...
			return {
				type: "git",
				repo: `https://${info.domain}/${info.user}/${info.project}`.replace(/\.git$/, ""),
				host: info.domain || "",
				path: `${info.user}/${info.project}`.replace(/\.git$/, ""),
				ref: info.committish || split.ref || undefined,
				pinned: Boolean(info.committish || split.ref),
