...
Feel free to use these tools whenever they would be helpful.`,
			model: getModel("openai", "gpt-4o-mini"),
			thinkingLevel: 0,
			messages: [],
		},
		tools: [createJavaScriptReplTool(), extractDocumentTool, convertToLlm],
		onStateChange: (state) => {
			if (state.status === "idle" && shouldSaveSession(state.messages)) {
				saveSession();
			}
		},
	});

	agentUnsubscribe = agent.onStateChange((state) => {
		if (state.status === "idle" && shouldSaveSession(state.messages)) {
			const newTitle = generateTitle(state.messages);
...
async function initApp() {
	...
	const onboardingComplete = await OnboardingDialog.open();
