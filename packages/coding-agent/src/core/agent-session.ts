...
import type {
	...
	AgentTool,
	ThinkingLevel,
} from "@boxiaolanya2008/pi-agent-core";
import type { AssistantMessage, ImageContent, Message, Model, TextContent } from "@boxiaolanya2008/pi-ai";
import { isContextOverflow, modelsAreEqual, resetApiProviders, supportsXhigh } from "@boxiaolanya2008/pi-ai";
...
export class AgentSession {
	...
	private _rebuildSystemPrompt(toolNames: string[], thinkingLevel?: ThinkingLevel): string {
	...
	async prompt(text: string, options?: PromptOptions): Promise<void> {
		...
		// Validate API key
		const apiKey = this._modelRegistry.getApiKey() || "";
		if (!apiKey) {
			throw new Error(`No API key found for ${this.model.provider}. Please check your configuration.`);
		}
	 ...
	 * Rebuilds system prompt to include thinking level instructions.
	 */
	setThinkingLevel(level: ThinkingLevel): void {
		...
		if (isChanging) {
			...
			// Rebuild system prompt with new thinking level
			this._baseSystemPrompt = this._rebuildSystemPrompt(this.getActiveToolNames(), effectiveLevel);
			this.agent.setSystemPrompt(this._baseSystemPrompt);
		}
	}
	...
	async switchSession(sessionPath: string): Promise<boolean> {
		...
		// Model is fixed during onboarding - no need to restore from session
		// The model is already set in the agent through modelRegistry
